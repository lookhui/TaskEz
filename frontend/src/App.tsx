        ...
        <main className="workspace">
          ...
          {snapshot ? (
            <div className="content-scroll">
              ...
              {activeTab === "autorun" ? <div className="single-panel-page"><DataPanel fill title={`启动项 (${filteredAutoruns.length})`} subtitle="Run 键、RunOnce 与启动目录，可直接禁用" search={searches.autorun} onSearchChange={(value) => updateSearch(setSearches, "autorun", value)} placeholder="筛选启动项..."><table className="data-table autorun-table"><thead><tr><th>名称</th><th>范围</th><th>位置</th><th>命令</th><th>操作</th></tr></thead><tbody>{filteredAutoruns.map((item) => <tr key={`${item.scope}-${item.location}-${item.name}`}><td>{item.name}</td><td><StatusPill label={item.scope} tone={item.scope === "Machine" ? "warning" : "attention"} /></td><td className="mono">{item.location}</td><td className="mono wrap-cell">{item.command}</td><td><button className="inline-action" disabled={actionKey === `autorun-${item.scope}-${item.name}`} onClick={() => void handleDisableAutorun(item)} type="button">禁用</button></td></tr>)}</tbody></table>{!filteredAutoruns.length ? <EmptyState text="没有匹配的启动项。" /> : null}</DataPanel></div> : null}
              {activeTab === "monitor" ? (
